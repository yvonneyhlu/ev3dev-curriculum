...
def main():
    ...
    while time_s != 0:
        ...
        speed = int(input("Enter the speed(dps): "))

        distance = int(input("Distance to travel (inches): "))
        distance_inch = distance * 90
        left_motor.run_to_rel_pos(position_sp = distance_inch, speed_sp = speed, stop_action=ev3.Motor.STOP_ACTION_BRAKE)
        right_motor.run_to_rel_pos(position_sp=distance_inch, speed_sp=speed, stop_action=ev3.Motor.STOP_ACTION_BRAKE)
